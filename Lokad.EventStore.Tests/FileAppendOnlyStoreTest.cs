...
using Lokad.EventStore.Files;
using NUnit.Framework;

namespace Lokad.EventStore.Tests
{
    public class FileAppendOnlyStoreTest
    {
        readonly string _storePath = Path.Combine(Path.GetTempPath(), "Lokad-CQRS");
        FileAppendOnlyStore _store;
        const int DataFileCount = 10;
        const int FileMessagesCount = 5;
        ...
        void CreateCacheFiles()
        {
            ...
            {
                using (
                    var stream = new FileStream(Path.Combine(_storePath, index + ".dat"), FileMode.OpenOrCreate,
        ...
        public void load_cache_when_exist_empty_file()
        {
            //write frame
            using (
                var stream = new FileStream(Path.Combine(_storePath, "0.dat"), FileMode.OpenOrCreate, FileAccess.Write,
                    FileShare.ReadWrite))
